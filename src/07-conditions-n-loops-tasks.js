...
function findFirstSingleChar(str) {
  ...
  const strArr = noSpacesStr.split('');
  for (let i = 0; i < strArr.length; i += 1) {
    if (strArr.filter((char) => char === strArr[i]).length === 1) return strArr[i];
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const cardNumStr = ccn.toString();
  const type = cardNumStr.length % 2 === 0 ? 'even' : 'odd';
  for (let i = 0; i < cardNumStr.length; i += 1) {
    let digit = +cardNumStr[i];
    if (type === 'odd' ? i % 2 !== 0 : i % 2 === 0) {
      digit *= 2;
      if (digit > 9) {
        const [dig1, dig2] = digit.toString().split('');
        digit = +dig1 + +dig2;
      }
    }
    sum += digit;
